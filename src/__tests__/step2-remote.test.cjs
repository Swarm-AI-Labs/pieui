...
// Verifies login route polls credentials endpoint and persists config + .env values.
test('login fetches credentials and writes .pie/config.json and .env', async () => {
    const projectDir = makeProjectDir('pieui-step2-login-success-')
    const requestUrls = []
    ...
    try {
        ...
        assertSucceeded(result, 'login should complete against local credentials API')
        assert.match(result.stdout, /Open link in browser:/)
        assert.match(result.stdout, new RegExp(`${baseUrl}/connect\\?code=`))

        const credentialPoll = requestUrls.find((url) =>
            /^\/credentials\?code=/.test(url)
        )
        assert.ok(credentialPoll, 'login should poll credentials endpoint')
        const code = credentialPoll.split('code=')[1]
        assert.ok(code && code.length === 32, 'login code should be 32 chars')

        const configPath = path.join(projectDir, '.pie', 'config.json')
        assert.ok(fs.existsSync(configPath))
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'))
        assert.equal(config.user_id, 'demo-user')
        assert.equal(config.project, 'demo-project')
        assert.equal(config.api_key, 'demo-key')

        const envPath = path.join(projectDir, '.env')
        assert.ok(fs.existsSync(envPath))
        const envContent = fs.readFileSync(envPath, 'utf8')
        assert.match(envContent, /^PIE_USER_ID=demo-user/m)
        assert.match(envContent, /^PIE_PROJECT=demo-project/m)
        assert.match(envContent, /^PIE_API_KEY=demo-key/m)
    } finally {
        await stopServer(server)
    }
})
